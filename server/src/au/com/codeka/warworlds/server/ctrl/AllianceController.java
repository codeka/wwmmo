...
public class AllianceController {
  ...
  public List<Alliance> getAlliances(boolean hideDead) throws RequestException {
    try {
      return db.getAlliances(hideDead);
    } catch (Exception e) {
  ...
  public int addRequest(AllianceRequest request) throws RequestException {
    ...
    if (request.getRequestType() == BaseAllianceRequest.RequestType.CHANGE_DESCRIPTION) {
      String newDescription = NameValidator.validate(
  ...
  private static class DataBase extends BaseDataBase {
    ...
    public List<Alliance> getAlliances(boolean hideDead) throws Exception {
      String sql = "SELECT alliances.*," +
          ...
          " ORDER BY name ASC";
      if (hideDead) {
        sql = "SELECT * FROM (" + sql + ") alliances WHERE alliances.is_active = 1 AND num_empires > 0";
      }
      try (SqlStmt stmt = prepare(sql)) {
        SqlResult res = stmt.select();
    ...
    public void createAlliance(Alliance alliance, int creatorEmpireID) throws Exception {
      String sql = "INSERT INTO alliances (name, creator_empire_id, created_date," +
          " bank_balance, image_updated_date, is_active) VALUES (?, ?, ?, 0, NOW(), 1)";
    ...
    public int addRequest(AllianceRequest request) throws Exception {
      ...
      sql = "INSERT INTO alliance_requests (" +
          ...
          " votes, target_empire_id, amount, png_image, new_name, new_description)" +
          " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
      try (SqlStmt stmt = prepare(sql, Statement.RETURN_GENERATED_KEYS)) {
        ...
        stmt.setString(12, request.getNewDescription());
        stmt.update();
