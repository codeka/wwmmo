...
public class ErrorReportsController {
  ...
  private static class DataBase extends BaseDataBase {
    ...
    public void saveErrorReports(Messages.ErrorReports error_reports_pb) throws Exception {
      String sql = "INSERT INTO error_reports (report_date, empire_id, message, exception_class, context, source, data) " +
          " VALUES (?, ?, ?, ?, ?, ?, ?)";
      try (SqlStmt stmt = DB.prepare(sql)) {
        for (Messages.ErrorReport pb : error_reports_pb.getReportsList()) {
          ...
          stmt.setInt(6, pb.getSource().getNumber());
          stmt.setBytes(7, pb.toByteArray());
          stmt.update();
        }
      }
    }
