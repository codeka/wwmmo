...
public abstract class AllianceRequestProcessor {
  ...
  public static AllianceRequestProcessor get(Alliance alliance, AllianceRequest request) {
    switch (request.getRequestType()) {
      ...
      case CHANGE_DESCRIPTION:
        return new ChangeDescriptionRequestProcessor(alliance, request);
    }
  ...
  private static class ChangeDescriptionRequestProcessor extends AllianceRequestProcessor {
    public ChangeDescriptionRequestProcessor(Alliance alliance, AllianceRequest request) {
      super(alliance, request);
    }
    ...
    protected void onVotePassed(AllianceController ctrl) throws Exception {
      ...
      String sql = "UPDATE alliances SET description = ? WHERE id = ?";
      try (SqlStmt stmt = ctrl.getDB().prepare(sql)) {
        stmt.setString(1, mRequest.getNewDescription());
        stmt.setInt(2, mRequest.getAllianceID());
        stmt.update();
      }
    }
