...
public class FleetOrdersHandler extends RequestHandler {
  ...
  private void orderFleetSetStance(Star star, Fleet fleet, Messages.FleetOrder fleet_order_pb, Simulation sim) {
    ...
    if (stance == BaseFleet.Stance.AGGRESSIVE) {
      // If you just made this fleet aggressive, we'll pretend like it just arrived and start
      // attacking anybody nearby.
  ...
  private void orderFleetSplit(Star star, Fleet fleet, Messages.FleetOrder fleet_order_pb, Simulation sim)
    ...
    if (leftShips > totalShips || rightShips > totalShips) {
