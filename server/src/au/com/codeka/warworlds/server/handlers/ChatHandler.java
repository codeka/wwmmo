...
public class ChatHandler extends RequestHandler {
    ...
    protected void post() throws RequestException {
        ...
        String sql = "INSERT INTO chat_messages (empire_id, alliance_id, message, message_en, posted_date, conversation_id)" +
                    " VALUES (?, ?, ?, ?, ?, ?)";
        try (SqlStmt stmt = DB.prepare(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            if (inp_chat_msg_pb.hasConversationId() && inp_chat_msg_pb.getConversationId() > 0) {
                stmt.setInt(6, inp_chat_msg_pb.getConversationId());
                chat_msg_pb.setConversationId(inp_chat_msg_pb.getConversationId());
            } else {
                stmt.setNull(6);
            }

            stmt.update();
            chat_msg_pb.setId(stmt.getAutoGeneratedID());
        } catch(Exception e) {
            throw new RequestException(e);
        }
