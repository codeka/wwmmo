...
import au.com.codeka.common.protobuf.AnonUserAssociate;
...
public class AnonUserAssociateHandler extends RequestHandler {
  ...
  public void post() throws RequestException {
    AnonUserAssociate pb = getRequestBody(AnonUserAssociate.class);

    // Check to make sure there's not already an empire with that email address.
    if (new EmpireController().getEmpireByEmail(pb.user_email) != null) {
    ...
    if (!getSession().getActualEmail().endsWith("@anon.war-worlds.com")) {
      throw new RequestException(400, GenericError.ErrorCode.NotAnonymous,
          "Your empire is already associated with an account, and cannot be associated again.");
    }

    new EmpireController().associateEmpire(getSession().getEmpireID(), pb.user_email);
  }
}
