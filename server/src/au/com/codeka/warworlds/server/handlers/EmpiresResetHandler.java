...
public class EmpiresResetHandler extends RequestHandler {
  ...
  protected void post() throws RequestException {
    Messages.EmpireResetRequest reset_request_pb =
        getRequestBody(Messages.EmpireResetRequest.class);
    ...
    ArrayList<Integer> starIDs = new EmpireController().getStarsForEmpire(empireID,
        EmpireController.EmpireStarsFilter.Everything, null);
    ...
    if (new EmpireController().getEmpire(empireID).getPatreonLevel()
        != BaseEmpire.PatreonLevel.EMPIRE) {
      if (numStarsWithColonies >= 5 &&
          (!reset_request_pb.hasPurchaseInfo() || !reset_request_pb.getPurchaseInfo().hasSku())) {
