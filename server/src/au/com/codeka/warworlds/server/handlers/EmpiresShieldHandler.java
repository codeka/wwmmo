...
public class EmpiresShieldHandler extends RequestHandler {
  ...
  protected void get() throws RequestException {
    ...
    if (pngImage == null) {
      if (getRequest().getParameter("final") != null
          && getRequest().getParameter("final").equals("1")) {
        try {
          // if we're doing a "final" image for this guy, just create a coloured image based on his
          // key
  ...
  protected void put() throws RequestException {
    ...
    try {
      ...
      throw new RequestException(
          400, GenericError.ErrorCode.InvalidImage, "Supplied image is not valid.");
    }
    ...
    if (empire.getPatreonLevel() == BaseEmpire.PatreonLevel.EMPIRE) {
      log.info("Empire level patron, not checking purchase.");
    } else {
    ...
    empire = new EmpireController().getEmpire(empireID);
    empire.toProtocolBuffer(empire_pb, true);
    setResponseBody(empire_pb.build());
  }

  private BufferedImage mergeShieldImage(
      BufferedImage shieldImage) throws ImageReadException, IOException {
