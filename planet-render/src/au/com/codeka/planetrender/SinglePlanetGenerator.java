...
/**
 * The {@see PlanetGenerator} uses this class to render a single planet image. It may (or may not) then combine
 * multiple planet images into on (e.g. for asteroids).
 */
public class SinglePlanetGenerator {
    ...
    public SinglePlanetGenerator(Template.PlanetTemplate tmpl, Random rand) {
        mPlanetOrigin = Vector3.pool.borrow().reset(tmpl.getOriginFrom());
        Vector3.interpolate(mPlanetOrigin, tmpl.getOriginTo(), rand.nextDouble());
