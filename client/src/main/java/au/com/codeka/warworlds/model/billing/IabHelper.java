...
/**
 * Provides convenience methods for in-app billing. Create one instance of this class for your
 * application and use it to process in-app billing operations. It provides synchronous (blocking)
 * and asynchronous (non-blocking) methods for many common in-app billing operations, as well as
 ...
 * <p>
 * After setup is complete, you may query whether the user owns a given item or not by calling
 * {@link #isOwned}, get all items owned with {@link #getOwnedSkus}, get an item's price with
 * {@link #getPrice}, amongst others (see documentation for specific methods).
 * <p>
 * Please notice that the object will only have knowledge about owned items; it will
 * automatically have information (such as price, description) for items that are not owned by the
 * user, because the server will not automatically provide those. In order to query information for
 * an item that's not owned (such as to display the price to the user before a purchase), you should
 * first bring the item's sku to the object's knowledge by calling {@link #addSku} and then perform
 * an inventory refresh by calling {@link #refreshInventory()} or its corresponding asynchronous
 * version {@link #refreshInventoryAsync}.
 * <p>
 * If you know the skus of all the items that you can possibly be interested in, you can call
 * {@link #addSku} for those items before {@link #startSetup}, and that way all the information
 * about them will be available from the start, with no need to refresh the inventory later.
 * <p>
 ...
 * operation at a time; attempting to start a second asynchronous operation while the first one
...
public class IabHelper {
   ...
   *                        app's base64-encoded public key in your application's page on Google
   *                        Play Developer Console. Note that this is NOT your "developer public
   *                        key".
   */
  ...
  /**
   * Helper method that converts a (possibly-null) into a (possibly-default-instance) proto message.
   */
  public static Messages.PurchaseInfo toProtobuf(String skuName, @Nullable Purchase purchaseInfo) {
    if (purchaseInfo == null) {
      return Messages.PurchaseInfo.getDefaultInstance();
    }
   ...
   * which will involve bringing up the Google Play screen. The calling activity will be
   ...
   *                    data when the purchase completes. This extra data will be permanently bound
   *                    to that purchase and will always be returned when the purchase is queried.
   */
  public void launchPurchaseFlow(
      Activity act,
      String sku,
    ...
    try {
      ...
      Bundle buyIntentBundle =
          mService.getBuyIntent(3, mContext.getPackageName(), sku, ITEM_TYPE_INAPP, extraData);
  ...
  public Inventory queryInventory(
      boolean querySkuDetails, List<String> moreSkus) throws IabException {
        ...
        for (Purchase purchase : purchases) {
          try {
            ...
            results.add(new IabResult(
                BILLING_RESPONSE_RESULT_OK, "Successful consume of sku " + purchase.getSku()));
          } catch (IabException ex) {
