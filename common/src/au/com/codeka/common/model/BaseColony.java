...
public abstract class BaseColony {
  ...
  /**
   * Gets the time it will take for the given number of troop carriers to convert this colony.
   */
  public float getPropagandaTime(float numTroopCarriers) {
    float defence = 0.25f * mPopulation * mDefenceBoost;
    if (defence < 1.0f) {
      defence = 1.0f;
    }
    if (numTroopCarriers < defence) {
      return 0.0f;
    }

    float fraction = 1.0f / (1.0f + (float) Math.log(numTroopCarriers / defence));
    ...
    return basePropagandaTime * fraction;
  }
