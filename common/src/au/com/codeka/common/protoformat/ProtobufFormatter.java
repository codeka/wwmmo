 ...
 */
package au.com.codeka.common.protoformat;
...
public abstract class ProtobufFormatter {
     ...
     * 
     * @param cs
     *            the character set to use by default, when unspecified
     */
     ...
     * @param message
     *            the protobuf message to format
     * @param output
     *            the stream to write the formatted message using the default
     *            charset
     * @throws IOException
     */
    public void print(final Message message, OutputStream output)
            throws IOException {
     ...
     *            the protobuf message to format
     * @param output
     *            the stream to write the formatted message
     * @param cs
     *            the character set to use
     * @throws IOException
     */
    abstract public void print(final Message message, OutputStream output,
            Charset cs) throws IOException;

    /**
     * @see print(UnknownFieldSet, OutputStream, Charset)
     * @param fields
     *            unknown fields to format
     * @param output
     *            output the stream to write the formatted message using the
     *            default charset
     * @throws IOException
     */
    public void print(final UnknownFieldSet fields, OutputStream output)
            throws IOException {
     ...
     *            unknown fields to format
     * @param output
     *            output the stream to write the formatted message
     * @param cs
     *            the character set to use
     * @throws IOException
     */
    abstract public void print(final UnknownFieldSet fields,
            OutputStream output, Charset cs) throws IOException;

    /**
     * Like {@code print()}, but writes directly to a {@code String} and returns
     * it.
     */
    public String printToString(final Message msg) {
        try {
            ByteArrayOutputStream out = new ByteArrayOutputStream();
            print(msg, out, defaultCharset);
            out.flush();
            return out.toString();
        } catch (IOException e) {
    ...
    abstract public void merge(final InputStream input, Charset cs,
            ExtensionRegistry extensionRegistry, final Message.Builder builder)
            throws IOException;
    ...
    public void merge(final InputStream input, final Message.Builder builder)
            throws IOException {

        merge(input, defaultCharset, ExtensionRegistry.getEmptyRegistry(),
                builder);
    }
