"""c2dm.py: Model for interfacing with the C2DM service."""
...
class Sender:
  ...
  def sendMessage(self, deviceRegistrationID, msg):
    ...
    request.add_header("Authorization", "GoogleLogin auth=%s" % (self.token_factory.getToken()))
    response = urllib2.urlopen(request)
    if response.code == 200:
      logging.info("Sent message to device: %s" % (response.read()))
      return True
    return False
