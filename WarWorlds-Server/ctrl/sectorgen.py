"""sectorgen.py Contains the code to generate sectors."""
...
def expandUniverse(immediate=False):
...
def generateSector(x, y):
  ...
  def _tx():
    ...
    sector.distanceToCentre = math.sqrt((x * x) + (y * y))
    sector.stars = []
    sector.put()
    return sector
  ...
  star_points = _poisson(SECTOR_SIZE - 64, SECTOR_SIZE - 64, 160, 80)
  for point in star_points:
    ...
    starTypeID = select(_starTypeBonuses)
  ...
  for star in sector.stars:
    planet_pbs = []
    numPlanets = select(_planetCountBonuses)
    for index in range(numPlanets):
      ...
      planet_pb.planet_type = select(type_bonuses)
...
def _poisson(width, height, min_distance, packing=30):
