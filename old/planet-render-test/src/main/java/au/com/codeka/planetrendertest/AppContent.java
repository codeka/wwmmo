...
public class AppContent extends JPanel {
  ...
  private void initialize() {
    ...
    backgroundColourCombo.addActionListener(new ActionListener() {
  ...
  private File getSamplesDirectory() {
    ...
    if (!dir.exists()) {
      try {
        dir = new File(
            AppContent.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath());
        dir = new File(dir.getParentFile().getParentFile(), "samples");
